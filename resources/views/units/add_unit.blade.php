...
@endif
<a href="/unit"><button class="btn btn-primary btn-small">VIEW</button></a>
    <!-- Begin Page Content -->
...
<div class="row">
    <div class="card shadow col-md-12">
            <div class="card-body">
                <form method="POST" action="/add-unit">
                    @csrf
    ...
    $(document).ready(function() {
        ...
        $(add_button).click(function(e){ //on add input button click
            ...
            if(x < max_fields){ //max input box allowed
                ...
                '<label> course</label><select class="form-control" name="adjArr[]" placeholder="Select course" /><br>@foreach($course as $value) <option value="{{$value->id}}">{{$value->course_name}}</option>@endforeach</select>  ' +
                    '<label> Unit Name</label><input type="text"  class="form-control"  placeholder="Enter value" name="acjArr[]"  /><br> ' +

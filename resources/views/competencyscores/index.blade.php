@extends('layouts.app')
@section('title', 'Competence scores')
@section('page-title', 'Competence scores')
...
<div class="container-fluid">
        ...
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">competence scores</h6>  </div>

            <div class="card-body">
            <div class="table-responsive">
                <a href="/add-competencyscore"><button class="btn btn-primary">ADD</button></a>
                <table class="table table-bordered school" id="dataTable"  cellspacing="0">
                    ...
                    <tbody>
                            ...
                            <tr id="row_{{$value->id}}">
                                 ...
                                 <td>{{$value->id}}</td> 
                                 <td>{{$value->score_name}}</td> 
                                 <td>{{$value->score_value}}</td>            
        ...
        <form action="/edit-competencyscore/{{$bb}}" method="post">
            @csrf
        ...
        function showDialog(id) {          
               ...
               console.log(id);
               var url =   "{{ url('/edit-competencyscore-modal') }}";
             //  var data = "id="+id;

               $("#myModalLabel").html("Edit competency scores");
        ...
        function edit(id){
            ...
            var data = $("form").serialize();
            var url =   "{{ url('/edit-competencyscore') }}";
...
<script src={{ asset("js/sweetalert.min.js")}}></script>
            ...
            $(document).on("click", "table.school tbody .unassigned", function(event) {
                var id = $(this).data('id').toString();
                    swal({
                    title: "Delete!",
                    text: "Are you sure you want to delete this competency score!",
                    icon: "error",
                    buttons: [true, "Delete"],
                    })
                    .then((willDelete) => {
                        if (willDelete) {
                            window.location.href="{{url('/delete-competencyscore/' )}}"+"/"+id;
                            swal({
                                title: "Success!",

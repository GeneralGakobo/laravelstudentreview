...
@section('page-title', 'Add Competency')
...
@endif
<a href="/competency"><button class="btn btn-primary btn-small">VIEW</button></a>
    <!-- Begin Page Content -->
...
<div class="row">
    <div class="card shadow col-md-12">
            <div class="card-body">
                <form method="POST" action="/add-competency">
                    @csrf
    ...
    $(document).ready(function() {
        ...
        $(add_button).click(function(e){ //on add input button click
            ...
            if(x < max_fields){ //max input box allowed
                ...
                '<label> Competency Group</label><select class="form-control" name="adjArr[]" placeholder="Select competency group" /><br>@foreach($competenciesGroups as $value) <option value="{{$value->id}}">{{$value->competency_group}}</option>@endforeach</select>  ' +
                    '<label> Conpetency Name</label><input type="text"  class="form-control"  placeholder="Enter value" name="acjArr[]"  /><br> ' +
